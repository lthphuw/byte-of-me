  ...
  try {
    ...
    if (me) {
      ...
      const content = [
        ...
        convertToText(me.quote) && `Phu's Favorite Quote:\n"${convertToText(me.quote)}"`,
        `Contact Information:\n${[me.email, me.phoneNumber].filter(Boolean).join(' | ')}`,
        `Links:\n${[me.github, me.linkedIn, me.portfolio, me.leetCode].filter(Boolean).join(' | ')}`,
        `=== End of Phu's Profile ===`,

        // Embed metadata in content
        `=== Metadata ===`,
        `Source: profile`,
        `Document Type: personal-profile`,
        `Name: ${me.name}`,
        `Keywords: Phu, personal information, about, bio, details, in short, abstract`,
      ]
        .filter(Boolean)
        .join('\n\n');
      ...
      rawDocs.push(
        new Document({
          id: `me-profile-${me.id || 'missing-user-id-' + Math.random().toString(36).slice(2)}`,
          pageContent: content,
          metadata: {
            source: 'profile',
            ...
            name: me.name,
            keywords: ['Phu', 'personal information', 'about', 'bio'],
          },
        }),
      );
    }
    ...
    for (const exp of experiences ?? []) {
      if (!exp.id) {
        console.warn(`Missing ID for experience at ${exp.company || 'Unknown'}`);
      }

      const techs = (exp as any).techStack?.map((t: string) => t).filter(Boolean).join(', ') || 'No technologies specified';
      const roleTitles = (exp as any).roles?.map((r: any) => r.title) || [];

      const roles = (exp as any).roles
        ?.map((r: any) => {
          const tasks = r.tasks?.map((t: string) => `- ${t}`).filter(Boolean).join('\n') || 'No tasks specified';
          if (!r.tasks || r.tasks.length === 0) {
            console.warn(`No tasks found for role "${r.title}" at ${exp.company || 'Unknown'}`);
          }
          return [
      ...
      const content = [
        ...
        `=== Metadata ===`,
        `Source: experience`,
        `Document Type: work-experience`,
        `Company: ${exp.company || 'Unknown'}`,
        `Technologies: ${techs}`,
        `Role Titles: ${roleTitles.join(', ') || 'No roles specified'}`,
        `Name: Phu`,
        `Keywords: Phu, work experience, job, employment, employee, work, ${exp.company || 'Unknown'}, Smart Loyalty${
          exp.company === 'Smart Loyalty' ? ', Software Engineer Intern 1, Software Engineer Intern 2' : ''
        }, ${exp.type || 'Unknown'}, ${exp.location || 'Unknown'}, ${techs.split(', ').join(', ')}, ${roleTitles.join(', ')}, ${
          (exp as any).roles
            ?.flatMap((r: any) =>
              r.tasks
      ...
      rawDocs.push(
        new Document({
          id: `exp-${exp.id || 'missing-exp-id-' + Math.random().toString(36).slice(2)}`,
          pageContent: content,
          metadata: {
            source: 'experience',
    ...
    for (const edu of educations ?? []) {
      ...
      const highlights = (edu as any).subItems
        ?.map((s: any) => `- ${convertToText(s.title)}: ${convertToText(s.message)}`)
        .filter(Boolean)
      ...
      const content = [
        ...
        `Name: Phu`,
        `Keywords: Phu, education, degree, study, ${edu.title}, ${edu.timeline}`,
      ]
        .filter(Boolean)
        .join('\n\n');
      ...
      rawDocs.push(
        new Document({
          id: `edu-${edu.id || 'missing-edu-id-' + Math.random().toString(36).slice(2)}`,
          pageContent: content,
          metadata: {
            source: 'education',
            ...
            title: convertToText(edu.title) || 'Unknown',
            name: 'Phu',
            keywords: ['Phu', 'education', 'degree', 'study', edu.title, edu.timeline].filter(Boolean),
          },
        }),
      );
    }
    ...
    for (const project of projects ?? []) {
      ...
      rawDocs.push(
        new Document({
          id: `project-${project.id || 'missing-project-id-' + Math.random().toString(36).slice(2)}`,
          pageContent: content,
          metadata: {
            source: 'project',
            ...
            title: project.title || 'Unknown',
            name: 'Phu',
            keywords: ['Phu', 'project', 'side project', project.title, ...tags.split(', '), ...techs.split(', ')].filter(Boolean),
          },
        }),
      );
    }
    ...
    if (techstacks && techstacks.length > 0) {
      const techstackId = `techstack-all`;
      const content = [
        `=== Phu's Technology Stack ===`,
        `List of technologies used by Phu in personal and side projects`,
        ...techstacks.map((tech) => `• ${tech.name} (Category: ${tech.group || 'N/A'})`),
        ...
        `Name: Phu`,
        `Tech Names: ${techstacks.map((t) => t.name).join(', ')}`,
        `Keywords: Phu, technology, tech stack, ${techstacks.map((t) => t.name).join(', ')}, ${techstacks.map((t) => t.group).filter(Boolean).join(', ')}`,
      ].join('\n\n');

      if (techstacks.some((t) => !t.id)) {
        console.warn(`Missing IDs for some tech stacks:`, techstacks.map((t) => ({ name: t.name, id: t.id })));
      }
      ...
      rawDocs.push(
        new Document({
          ...
          metadata: {
            ...
            keywords: ['Phu', 'technology', 'tech stack', ...techstacks.map((t) => t.name), ...techstacks.map((t) => t.group).filter(Boolean)],
          },
        }),
      );
    }
    ...
    return allChunks.map((doc, i) => {
      const parentId = doc.id || doc.metadata.id || 'missing-id-' + Math.random().toString(36).slice(2);
      return {
        ...doc,
