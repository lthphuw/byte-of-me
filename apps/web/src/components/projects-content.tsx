...
export function ProjectsContent({ projects }: ProjectsContentProps) {
  ...
  const categorizedProjects = useMemo(
    () =>
      projects.filter(
        (it) =>
          (!selectedTag ||
            Boolean(it.tags.find((tag) => tag.id === selectedTag))) &&
          (!selectedTechstack ||
            Boolean(it.techstacks.find((tag) => tag.id === selectedTechstack)))
      ),
  ...
  const showedProjects = useMemo(
    () =>
      categorizedProjects.filter(
        (it) => it.title.includes(prefix) || it.description?.includes(prefix)
      ),
