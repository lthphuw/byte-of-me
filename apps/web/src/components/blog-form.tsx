...
import { getPaginatedProject } from '@/lib/actions/dashboard/project/get-paginated-project';
import { getPaginatedTags } from '@/lib/actions/dashboard/tag/get-paginated-tags';
import { BlogFormValues, blogFormSchema } from '@/lib/schemas/blog.schema';
...
export function BlogForm({ initialData, onSubmit, loading }: BlogFormProps) {
  const { data: tagsData, isLoading: isTagLoading } = useQuery({
    queryKey: ['tags', 1],
    queryFn: () => getPaginatedTags(1, 100),
  });

  const { data: projectData, isLoading: isProjectLoading } = useQuery({
    queryKey: ['projects', 1],
    queryFn: () => getPaginatedProject(1, 100),
  });
