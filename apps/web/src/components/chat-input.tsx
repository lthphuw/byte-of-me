...
export default function ChatInput({ clearChat, onSend, loading, className }: ChatInputProps) {
    ...
    const handleSend = async () => {
        ...
        // Gửi truy vấn và token (nếu ở production) tới API server
        try {
            const response = await fetch("/api/verify", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ message: input.trim(), token: isProduction ? captchaToken : null }),
            });
            ...
            >
                ...
                <div className="mt-2 flex gap-3 justify-end">
                    ...
                    <Button
                        variant="default"
                        onClick={handleSend}
                        disabled={loading || !input.trim() || isRateLimited || (isProduction && !captchaToken)}
