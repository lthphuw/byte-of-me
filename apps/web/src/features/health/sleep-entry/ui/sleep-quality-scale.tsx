'use client';

import { useTranslations } from 'next-intl';

import { cn } from '@/shared/lib/utils';

const LEVELS = [1, 2, 3, 4, 5] as const;

/** The ramp, as bar heights. Five identical squares said nothing about which
 *  end was bad; a bar that grows with the level says it without needing a
 *  colour, which is the rule — colour is never the only carrier of meaning. */
const BAR_HEIGHT: Record<number, string> = {
  1: 'h-1.5',
  2: 'h-2.5',
  3: 'h-3.5',
  4: 'h-[1.125rem]',
  5: 'h-[1.375rem]',
};

/**
 * How the night felt, 1–5.
 *
 * Three things the five identical squares did not do. It NAMES the level in
 * words beside the group ("Fair", "Tạm ổn") so the number is not the only clue
 * what a 3 means; it RAMPS — the bar inside each button grows with the level,
 * and every bar up to the chosen one fills, so the control reads left-to-right
 * as poor→great before anything is selected; and it gives the selection a real
 * state.
 *
 * That state is an INVERSION, not a tint. On this palette every token is 0%
 * saturation (§14), so a tinted fill of the kind a branded app would use lands
 * within a few percent of the unselected surface and is not a state at all.
 * The chosen button flips to `bg-primary` with `text-primary-foreground`, its
 * bar flips with it, and the word above changes — three cues, none of them a
 * hue, on top of `aria-pressed`.
 *
 * Toggle buttons rather than a radio group, and `role="group"` rather than
 * `radiogroup`, because quality is OPTIONAL and clearable: tapping the current
 * value returns to "not answered", which a radio group has no gesture for.
 * Each button carries its own `aria-pressed` and its own label, so the pressed
 * state is announced without relying on the ramp.
 *
 * 48px targets in a 5-up grid with an 8px gap — the phone minimum, and the
 * reason this is a grid rather than a row of chips that could wrap mid-scale.
 */
export function SleepQualityScale({
  value,
  onChange,
}: {
  value: number | null;
  onChange: (value: number | null) => void;
}) {
  const t = useTranslations('dashboard.health');

  // Literal keys, one per level. next-intl's generated declarations only
  // type-check literals, so `t(`sleep.qualityLevel${n}`)` would type-check
  // against nothing and happily ship a key that does not exist — the same
  // reason the factor labels below are spelled out.
  const levelLabels: Record<number, string> = {
    1: t('sleep.qualityLevel1'),
    2: t('sleep.qualityLevel2'),
    3: t('sleep.qualityLevel3'),
    4: t('sleep.qualityLevel4'),
    5: t('sleep.qualityLevel5'),
  };

  return (
    <div className="space-y-2">
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-sm font-medium">{t('sleep.quality')}</span>
        {/* The word, live. It is the half of this control that survives with
            no colour and no shape — and it is what makes "4" mean something
            on a screen the author is reading half awake. */}
        <span
          aria-live="polite"
          className={cn(
            'truncate text-sm',
            value === null
              ? 'text-muted-foreground'
              : 'font-medium text-foreground'
          )}
        >
          {value === null ? t('sleep.qualityNone') : levelLabels[value]}
        </span>
      </div>

      <div
        role="group"
        aria-label={t('sleep.quality')}
        className="grid grid-cols-5 gap-2"
      >
        {LEVELS.map((level) => {
          const isActive = value === level;
          const isReached = value !== null && level <= value;

          return (
            <button
              key={level}
              type="button"
              aria-pressed={isActive}
              aria-label={
                isActive
                  ? t('sleep.qualityClear')
                  : `${t('sleep.qualityValue', { value: level })} — ${
                      levelLabels[level]
                    }`
              }
              // Tapping the current value clears it. Quality is optional and
              // never blocks a save, so there has to be a way back to "not
              // answered" once a stray tap has answered it.
              onClick={() => onChange(isActive ? null : level)}
              className={cn(
                'flex h-12 flex-col items-center justify-end gap-1.5 rounded-md border pb-1.5 pt-1',
                'transition-colors duration-200',
                'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background',
                isActive
                  ? 'border-primary bg-primary text-primary-foreground'
                  : 'border-border bg-card hover:border-primary/40 hover:bg-muted'
              )}
            >
              {/* Three fills, not two: the chosen button is inverted, so its
                  bar has to invert with it or it disappears into the fill.
                  Reached-but-not-chosen stays `bg-foreground`; unreached is
                  the faint one, which is what makes the ramp readable at rest. */}
              <span
                aria-hidden
                className={cn(
                  'w-1.5 rounded-full transition-colors duration-200',
                  BAR_HEIGHT[level],
                  isActive
                    ? 'bg-primary-foreground'
                    : isReached
                    ? 'bg-foreground'
                    : 'bg-muted-foreground/30'
                )}
              />
              <span
                className={cn(
                  'text-xs leading-none tabular-nums',
                  isActive
                    ? 'font-semibold text-primary-foreground'
                    : 'font-medium text-muted-foreground'
                )}
              >
                {level}
              </span>
            </button>
          );
        })}
      </div>
    </div>
  );
}
