...
export function MainNav({ items, children, minimized }: MainNavProps) {
  ...
  const handleToggleMenu = () => {
    if (iconRef.current) {
      ...
      setOriginPosition({ x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 })
    }
  ...
  return (
    <div className="flex gap-6 md:gap-10">
      <Link href="/" className="hidden items-center space-x-2 md:flex">
        <motion.div variants={logoAnimVariants} animate={minimized ? "minimized" : "initial"} className="hidden items-center gap-2 md:flex">
          <Icons.logo />
